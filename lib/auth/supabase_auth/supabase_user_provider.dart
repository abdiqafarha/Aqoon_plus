...
class AqoonPlusSupabaseUser extends BaseAuthUser {
  AqoonPlusSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> aqoonPlusSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AqoonPlusSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
